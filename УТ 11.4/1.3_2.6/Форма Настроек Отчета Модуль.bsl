...
Функция  ВалютаВидаЦены(ВидЦены)    
	...
	Возврат ВидЦены.Значение.ВалютаЦены ;
...
Процедура Подключаемый_ЭлементНастройки_ПриИзменении(Элемент)
	...
	ИначеЕсли ТипЗнч(ЭлементНастройки) = Тип("ЗначениеПараметраНастроекКомпоновкиДанных")
		И ЭлементНастройки.Параметр = Новый ПараметрКомпоновкиДанных("ВидЦены") Тогда 
		
		ПараметрВалюта = Отчет.КомпоновщикНастроек.Настройки.ПараметрыДанных.Элементы.Найти("Валюта");  
		ПараметрВалюта.Значение = ВалютаВидаЦены(ЭлементНастройки);    
		
		ПользовательскаяНастройкаВалюта = Отчет.КомпоновщикНастроек.ПользовательскиеНастройки.Элементы.Найти(ПараметрВалюта.ИдентификаторПользовательскойНастройки);
		Если ПользовательскаяНастройкаВалюта <> Неопределено   Тогда 
			
			ПользовательскаяНастройкаВалюта.Значение = ПараметрВалюта.Значение;
